---
title: 'Building UI components correctly: A Practical Guide'
date: '2026-01-01'
...
In this post, I aim to go through few of those fundamentals, the things AI can help you type, but not truly reason about for you.
...
Consider a seemingly trivial task: "Add delete functionality for list items which is being rendered in a drawer".
...
- Label is automatically associated with then input when provided.
...
With form libraries like React Hook Form, this component plugs in with almost no extra work.legacy.

This section should do what you want: clearly show both `<select>` and a custom dropdown, and push virtualization to the NFUI doc.

---

## Dropdown / Combobox – native first, custom when needed
...
component, we need to add significant amount of code to mimic browser.
...
To summarize, following are the things that I consider while creating a component.
...
the modal click handlers for `confirm` and `cancel` button is wired. Finally a `onConfirm` and `onClose` callbacks
are exposed as prop. This when done at multiple places, leads to major code bloat.
...
This is the issue with declarative approach. And so to solve this I took an imperative approach.
...
All `no-frills-ui`'s stackable components uses this layer manager to get their layer.

Now lets re-consider the example task that I mentioned in this blog post.

> Add delete functionality for list items which is being rendered in a drawer.

For this, the developer can directly render the button in each list time.
When the delete button is clicked, imperative `ConfirmDialog` can be shown.
`ConfirmDialog` will use `LayerManager` to stack above the `Drawer`.
And once user confirms, the delete API can be called or simply close the dialog
if user rejects, and the focus goes back to drawer in either case.

This is what a properly engineered set of components can enable. AI can generate code
for all these faster than any human but only a human think of the bigger picture and
plan ahead to achieve the goal with fewer lines of code and fewer lines of code means
fewer bugs and better maintainability.

Under the hood, NFUI also:

- Uses GitHub Actions to run tests and lint on every PR.
- Uses tag-protected releases, so only admins can publish versions.
- Publishes Storybook to GitHub Pages and archives docs per version.

## Where to explore further

If you are interested then your can checkout `no-frills-ui` [repository](https://github.com/pushkar8723/no-frills-ui) or
