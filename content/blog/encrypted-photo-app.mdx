---
title: Creating an End-to-End Encrypted Web App
date: "2021-03-13"
...
And so all our private moments are available for corporate companies to train their ML model. Thus he set
...
already in place. Even a working native application was there. I liked the idea and agreed to help him out.
Putting my 6+ years of experience in development to use, I grossly underestimated the size of the project. 😝

<p align="center">
    <img src="https://user-images.githubusercontent.com/2996493/110342773-cc1b9c80-8051-11eb-8955-3305325a33e0.png" alt="Estimations" width="50%"/>
</p>
...
The main challenge of such an application is that whatever heavy processing we do on the server in a
regular photos/videos application, cannot be done here as by the time a file reaches the server, it is
already encrypted. Thus everything has to be done on client-side. Fortunately, most of the problem was
solved for the app and the only re-implementation for the web was required. However, since web apps don't
have access to the filesystem (without using an experimental API), and encryption / decryption of photos
and videos would be a taxing process, I had to use every tool that I have to ensure performance.
...
for this as it is accessible everywhere (main thread, webworker and service workers) and also has responds with
...
// Check if we already have thumbnail for the file id
...
we settled for [PhotoSwipe](https://photoswipe.com/). It fit most of our use case. The only part missing
...
And so we kept looking. By luck I stumbled on [this awesome podcast]([https://www.youtube.com/watch?v=G9PpImUEeUA](https://www.youtube.com/watch?v=G9PpImUEeUA)). 
...
I am happy with the progress that we have made with the project. I knew about these technologies that I used
from various conference talk / Youtube videos. But implementing them was a completely different ball game.
Learned about WebWorkers, Comlink, CacheStorage, and ReadableStream. Worked with multiple libraries and filled in
...
[MediaSource](https://developer.mozilla.org/en-US/docs/Web/API/MediaSource). However, I am currently stuck on
specifying the codec for videos. Do let me know if you know about something that can help me out with it.
I would love to hear from you. 😊

I would also like to add service worker for offline caching and convert it to a PWA.

You can check out the app at [photos.ente.io]([https://photos.ente.io/](https://photos.ente.io/)) or can also
checkout the [code]([https://github.com/ente-io/bada-frame](https://github.com/ente-io/bada-frame)).