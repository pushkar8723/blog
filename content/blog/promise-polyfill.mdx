...
Lets be honest, nobody needs to write a polyfill for promise these days. Most of us only look for it when we
...
Lets start by defining a basic Promise class.
...
Before we start Implementing these methods, we must define how the Promise works. Lets list down the key behaviors:
...
behaviors as Promise having a higher priority. If this is something new to you and you want to understand
...
the resolved value fo the returned Promise. Following snippet will demonstrate this.
```JavaScript
...
Next lets implement the resolve and reject function. For this we need do define few variables in our class.
...
Now lets focus on resolve. As we know, we need to push the execution of resolve
