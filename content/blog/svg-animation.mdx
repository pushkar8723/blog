...
But I was like, GIFs are so 2010 and SVGs are the new cool. Only one problem, I don't know a thing about SVGs.
I mean yes I have used it for icons and some illustrations. Sometimes generated one from some utility and snap
...
etc. Then simply write CSS to animate it. Neat! So now I can just move a logo on the screen and have
boomers look at it for hours!
...
do a moving sine wave. And to compensate for my lack of creativity, I learned another tag called `path`, which
...
_(no pun intended)_. It was not hard to get _(pun intended)_. There are 6 different types of commands.
...
The following image will help you understand how it works.
...
shown in the first image. Then I can slightly move the tangents inwards to dampen the curve. Finally, I
closed the path using lines. I moved the center point slightly to have curves that are slightly different.
But making sure that the angle the tangents make with the vertical axis is the same. This ensures that the 
curve looks continuous when I animate it. Finally, I ended up with the path as described below.

```HTML
<path d="M0,160 C320,300,420,300,740,160 C1060,20,1120,20,1440,160 V0 H0" />
```
If you are having trouble with understanding the path described above, look at the section in the image
...
Although, we can now just copy-paste the above path and create another with slight variations and call it
...
![final](https://user-images.githubusercontent.com/2996493/87861537-d4a95080-c964-11ea-94c3-1eb5f300e100.png)
...
Finally, I added two more similar waves, the only difference is that I used `scaleY` transform CSS property
