...
github: pushkar8723/no-frills-ui
parent: /projects/
priority: 4
---
...
from actual `<button>`, `<input>` and `<select>` elements. This means you get
...
**Platform inconsistencies:** Some native elements like `<option>` can't be fully styled, so their
appearance will vary across browsers and operating systems. If you need pixel-perfect design control,
you'll want a library that builds everything from scratch.

**Small community:** There's no Discord server, no Stack Overflow answers, no battle-tested solutions to edge cases.
You're pretty much on your own if you hit issues.
